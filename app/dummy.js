...
// ─── BROWSE PAGE ───
function BrowsePage({ cooks, profile, userLocation, loading, error, onViewCook, onViewProfile, onLogout, toast }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: 32, maxWidth: 1200, margin: "0 auto" }}>
        {loading && (
          <div style={{ padding: "12px 0", textAlign: "center", color: "var(--text-secondary)" }}>Loading nearby cooks...</div>
        )}
        {error && (
          <div style={{ padding: "12px 0", textAlign: "center", color: "var(--red)" }}>{error}</div>
        )}
...
export default function HomePage() {
  ...
  const searchLocation = useMemo(() => (profile ? { lat: profile.latitude || 28.6139, lng: profile.longitude || 77.2090 } : defaultLocation), [profile]);
  ...
  const fetchCooks = useCallback(async () => {
    ...
    if (error) {
      ...
      setCookError(error.message || "Unable to load cooks");
      return;
    }
  ...
  const handleOnboardComplete = async (fullProfile) => {
    try {
      ...
      const { error } = await supabase.from("profiles").insert({
        ...
        latitude: fullProfile.latitude || null,
        longitude: fullProfile.longitude || null,
        cuisines: fullProfile.cuisines || [],
        availability: fullProfile.availability || {},
        price_range: fullProfile.price_range || null,
        service_radius_km: fullProfile.service_radius_km || 0,
        rating: 0,
        reviews_count: 0,
        verified: false,
      });
      ...
      if (fullProfile.role === "cook") {
        setCooks(prev => [...prev, { ...fullProfile, id: authUser.id, rating: 0, reviews_count: 0, verified: false }]);
        setView("cook_dashboard");
      } else {
        setView("browse");
      }
      ...
      toast("Failed to save profile. You can continue offline.", "error");
      // Still update local state even if Supabase save fails, so user can continue
      setProfile(fullProfile);
      if (fullProfile.role === "cook") { setCooks(prev => [...prev, { ...fullProfile, id: authUser.id }]); setView("cook_dashboard"); }
      else setView("browse");
    }
  };
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          price_range: updated.price_range || null,
          service_radius_km: updated.service_radius_km || 0,
        })
        .eq("id", authUser.id);
      ...
      setProfile(updated);
      if (updated.role === "cook") {
        setCooks(prev => prev.map(c => c.id === authUser.id ? updated : c));
      }
      toast("Profile updated successfully!", "success");
    } catch (err) {
      console.error("Error updating profile:", err);
      toast("Failed to update profile. Changes saved locally.", "error");
      // Still update local state
      setProfile(updated);
      if (updated.role === "cook") setCooks(prev => prev.map(c => c.id === updated.id ? updated : c));
    }
  };
  const handleDeleteAccount = async () => {
    try {
      ...
      if (profile?.role === "cook") {
        setCooks(prev => prev.filter(c => c.id !== authUser.id));
      }
      setProfile(null);
      setAuthUser(null);
      setView("auth");
      ...
      toast("Failed to delete account from database. Please try again.", "error");
      // Still logout locally even if Supabase delete fails
      if (profile?.role === "cook") setCooks(prev => prev.filter(c => c.id !== profile.id));
      setProfile(null);
      setAuthUser(null);
      setView("auth");
    }
    ...
    <ToastProvider>
      {(toast) => (
        <>
          ...
          {view === "browse" && <BrowsePage cooks={cooks} profile={profile} userLocation={userLocation} loading={loadingCooks} error={cookError} onViewCook={handleViewCook} onViewProfile={handleViewProfile} onLogout={handleLogout} toast={toast} />}
